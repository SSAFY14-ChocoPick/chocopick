...
class BeaconRepository(
    ...
    private var entryShownOnce = false
    private var lastNotifiedAt = 0L

    private val exitDistanceM = 1.5

    fun start() = ds.start()
    fun stop() = ds.stop()

    fun enterEvents(): Flow<Unit> =
        ds.distances().transform { d ->
            val now = d.timestamp

            // ✅ 앱 실행 동안 1번만: 이미 보여줬으면 절대 emit 안 함
            if (entryShownOnce) {
                // inside 상태 업데이트는 선택(원하면 유지)
            ...
            // ✅ 이미 inside면 "입장" 판정 필요 없음. exit만 본다.
            if (inside) {
